...
// CreateGroup creates a new group with the given index as the first member at place 0
func (g *Groups) CreateGroup(indexID int) (int, error) {
	if !g.isLoaded {
	...
	// Check if index is already assigned to another group
	if existingGroup, ok := g.indexToGroup[indexID]; ok {
		return 0, fmt.Errorf("index ID %d is already assigned to group %d", indexID, existingGroup)
	}

	id := g.nextGroupID
	g.nextGroupID++

	// Create group with first member at place 0
	member := Member{
		IndexID: indexID,
		Place:   0,
	}
	g.groups[id] = []Member{member}
	g.indexToGroup[indexID] = id
	g.isPersisted = false
...
// Truncate clears all groups and truncates the storage
func (g *Groups) Truncate() error {
	if !g.isLoaded {
		if err := g.load(); err != nil {
			return err
		}
...
// Close flushes any pending changes and closes the groups manager
